 ...
 */
void jugarPartida(char tablero[TAMANO_TABLERO][TAMANO_TABLERO]) {
    // Inicializa el tablero con números
    inicializarTablero(tablero);
    // El jugador que empieza es 'X'
    char jugadorActual = 'X';
    // Variable para saber si el juego ha terminado
    bool juegoTerminado = false;
    // Variable para guardar la casilla elegida por el jugador
    int casillaElegida;

    // Bucle principal de la partida (se repite mientras el juego NO haya terminado)
    while (!juegoTerminado) {
        // Muestra el tablero actual
        mostrarTablero(tablero);

        // Bucle para asegurar una jugada válida
        do {
            // Obtiene la casilla que quiere el jugador
            casillaElegida = obtenerJugada(jugadorActual);
        // Repite si la jugada no es válida (casilla ocupada o fuera de rango implícito)
        } while (!validarJugada(tablero, casillaElegida));

        // Una vez validada, realiza la jugada
        realizarJugada(tablero, casillaElegida, jugadorActual);

        // Verifica si el jugador que acaba de mover ha ganado
        if (verificarGanador(tablero, jugadorActual)) {
            ...
            juegoTerminado = true;
        // Si no ganó, verifica si hay empate
            ...
            // Marca el juego como terminado
            juegoTerminado = true;
        // Si no ganó ni hubo empate...
        } else {
            // ...cambia el turno al otro jugador
            cambiarTurno(jugadorActual);
        }
    } // Vuelve al inicio del bucle while si juegoTerminado es false
}
